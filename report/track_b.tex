...
%
% This file is \input{}-ed from report/main.tex so that the agent does
% not have to load the full main.tex into its context window while
% iterating on Track B. Keep the Phase numbering in lock-step with the
% rest of the report.
% =====================================================================

\section{Phase 4: Track B with V-JEPA 2}
\label{sec:phase4}
...
The dependencies grow by exactly one entry --- \code{transformers}~$\geq
4.45$, which is the release that ships \code{VJEPA2Model}:

\begin{lstlisting}[style=shell]
uv add 'transformers>=4.45'
\end{lstlisting}

After that, training, evaluation and submission all go through the
existing entrypoints:

\begin{lstlisting}[style=shell]
% Train (V-JEPA 2 ViT-L / 256 / 16 frames, frozen + attentive probe)
PYTHONPATH=src uv run python -m smth2smth.pipelines.train \
    track=b experiment=track_b_vjepa2

% Evaluate on the official val/ split
...
% Submit with TTA flip + auto class-pair remap
...
\subsection{Phase 4.1: zero-shot evaluation with the SSv2 head}
\label{sec:phase4:zero_shot}
...
\subsection{Phase 4.2: heavy data augmentation for the V-JEPA 2 probe}
\label{sec:phase4:heavy_aug}
...
(Sec.~\ref{sec:phase4:heavy_aug}) flagged as the dominant cost.
...
\subsection{Phase 4.3: overnight ViT-g 384 heavy-aug run}
\label{sec:phase4-vitg384}
...
\subsection{Phase 4.4: multi-query probe and PEFT LoRA}
\label{sec:phase4-peft}
